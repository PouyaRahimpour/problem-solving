...
int n; const int mxn =2e5+20;
int a[mxn];
bool ok(int x) {
    int x1 = upper_bound(a, a+n, a[0]+2*x)-a;
    int x2 = upper_bound(a, a+n, a[x1]+2*x)-a;
    int x3 = upper_bound(a, a+n, a[x2]+2*x)-a;
    return x3>=n || x2>=n || x1>=n;
}
void solve() {
    cin >> n;
